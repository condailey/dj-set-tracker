...
const getConnection = () => mysql.createConnection({
    ...
    database: process.env.MYSQLDATABASE || 'dj_set_tracker',
});
    ...
    Mutation: {
        ...
        updateTrack: async (_: any, args: { id: string; input: any }) => {
            ...
            await connection.execute(
                'UPDATE tracks SET title = ?, artist = ?, bpm = ?, musical_key = ?, genre = ?, energy_level = ?, notes = ? WHERE id = ?',
        ...
        rateTransition: async (_: any, args: { input: any }) => {
            ...
            const connection = await getConnection();
            const [result] = await connection.execute(
                'INSERT INTO transitions (set_id, from_track_id, to_track_id, rating, notes) VALUES (?, ?, ?, ?, ?)',
